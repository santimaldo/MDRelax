...
# %%============================================
### g(r) St-St INTRAmolecular NPT

# path_Gromacs = "/home/santi/mendieta/DME_PS/"
# u = mda.Universe(f"{path_Gromacs}NPT.tpr", f"{path_Gromacs}NPT.trr")
# cations = u.select_atoms(f"name Li*")
# anions = u.select_atoms(f"name S6*")
# rdfs = []
...
# plt.figure(1)
# plt.plot(bins, rdfs, 'o-', label="S6t-S6t")
# plt.xlabel(r"$S_{6t}-S_{6t}$ [$10^{-10}$ m]")
# plt.ylabel(r"$g(r)$")
# plt.legend()
...
for ii in range(6,11):
    ...
    lithiums = u.select_atoms(f"name Lij")
    oxygens = u.select_atoms(f"name OW")    
    rdf = InterRDF(lithiums, oxygens,
                   nbins=560, range=[0.5,7.5])
    rdf.run()    
    binss.append(rdf.bins[1:])
    rdfs.append(rdf.rdf[1:])
rdfs = np.array(rdfs)
#%%
plt.figure(1)
for ii in range(5):
    plt.plot(binss[ii], rdfs[ii,:], '-', label=f"run={ii+6}")
plt.plot(binss[0], np.mean(rdfs, axis=0), 'k--', label=f"Mean over runs")
plt.xlabel(r"$Li-O$ [$10^{-10}$ m]")
plt.ylabel(r"$g(r)$")
plt.xlim([1,7])
plt.legend()
# %%
