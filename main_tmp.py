...
for factor_q in [1,0.5,0.1,0.9,0.75]:
    ...
    # t0 = time.time()
    # calculate_ACF(path_MDrelax+f"factor-q_{factor_q:.2f}/",
    #             savepath = path_MDrelax+f"factor-q_{factor_q:.2f}/",
    #             species = [cation, anion, solvent],              
    #             Ncations = Ncations,
    #             runs_prefix = runs_prefix,
    #             runs_suffix = runs_suffix,
    ...
    t0 = time.time()
    plot_ACF(path_MDrelax+f"factor-q_{factor_q:.2f}/",
                savepath = path_MDrelax+f"factor-q_{factor_q:.2f}/",
                species = [cation, anion, solvent],              
                salt = salt,
                Ncations = Ncations,
                ...
                runs_suffix = runs_suffix,
                max_tau =  10,
                fignum=np.random.randint(1,786534278))             
